...
BOT_COMMANDS = [
    ...
    ("lightning", "Toggle Lightning Mode"),
...
class TelegramChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        self._app.add_handler(CommandHandler("reasoning", self._on_reasoning))
    ...
    async def _on_reasoning(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle /reasoning command — show reasoning-level picker."""
        if not update.message or not update.effective_user:
            return
        user = update.effective_user
        chat_id = update.message.chat_id
    ...
    async def _on_lightning(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle /lightning command — show Lightning Mode toggle."""
        if not update.message or not update.effective_user:
            return
        user = update.effective_user
        chat_id = update.message.chat_id
    ...
    async def _on_callback_query(
        ...
        elif query.data.startswith("reasoning_level:"):
            value = query.data.split(":", 1)[1]
            await self._handle_message(
                sender_id=sender_id,
                chat_id=str(chat_id),
                content=f"/reasoning {value}",
                metadata={
        ...
        elif query.data.startswith("lightning_mode:"):
            value = query.data.split(":", 1)[1]
            await self._handle_message(
                sender_id=sender_id,
                chat_id=str(chat_id),
                content=f"/lightning {value}",
                metadata={
