...
function TriggerHistory({ hookId }: { hookId: string }) {
  ...
  return (
    <div className="min-w-0 max-w-full overflow-hidden rounded-[4px] border border-line bg-raised">
      {data.map((rec, i) => (
        <div key={i} className="min-w-0 border-b border-line last:border-0">
          <button
            onClick={() => setOpen(open === i ? null : i)}
            className="flex min-w-0 w-full flex-col gap-1 overflow-hidden px-3 py-2.5 text-left hover:bg-raised2/50"
          >
            <span className="font-mono text-[10.5px] text-body">{timeOf(rec)}</span>
            <span className="block w-full min-w-0 truncate font-mono text-[10px] text-muted">{preview(rec)}</span>
          </button>
          {open === i && (
...
function HookDetail({
  ...
  return (
    <div className="flex h-full min-w-0 flex-col">
      ...
      <div className="min-h-0 min-w-0 flex-1 space-y-5 overflow-x-hidden overflow-y-auto p-4 lg:p-6">
          ...
          <>
            ...
            <div className="min-w-0">
              <button
                ...
                className="mb-2 flex w-full items-center gap-2 rounded-[3px] py-1 text-left hover:text-ink"
              >
                <SectionLabel>Trigger history</SectionLabel>
                <span className="ml-auto font-mono text-[10px] text-muted">
                  {historyOpen ? 'Hide' : 'Show'} {historyOpen ? '▴' : '▾'}
                </span>
              </button>
              {historyOpen && <TriggerHistory hookId={hook.id} />}
            </div>
          </>
        )}
      </div>
